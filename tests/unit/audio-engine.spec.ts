...
describe("MamuteEngine — applyGains (G1–G6)", () => {
  ...
  test("G6 booth, cueMix e CUE roteiam o monitor", async () => {
    ({ engine } = await createTestEngine());
    engine.setBooth(0.11);
    engine.setCueMix(0.22);
    expect(engine.snapshot.booth).toBe(0.11);
    expect(engine.snapshot.cueMix).toBe(0.22);
    expect(engine.__test__.booth()?.gain.value).toBe(0.11);
    expect(engine.__test__.master()?.gain.value).toBe(engine.snapshot.master);
    expect(engine.__test__.masterToPhones()?.gain.value).toBe(1);
    expect(engine.__test__.cueToPhones()?.gain.value).toBe(0);

    engine.setCueMonitor("a", true);
    expect(engine.__test__.decks()?.a.gain.gain.value).toBe(0);
    expect(engine.__test__.decks()?.a.cueSend.gain.value).toBe(1);
    expect(engine.__test__.masterToPhones()?.gain.value).toBeCloseTo(1 - 0.22, 5);
    expect(engine.__test__.cueToPhones()?.gain.value).toBeCloseTo(0.22, 5);

    engine.setCueMonitor("b", true);
    expect(engine.__test__.decks()?.b.gain.gain.value).toBe(0);
    expect(engine.__test__.decks()?.b.cueSend.gain.value).toBe(1);
    expect(engine.__test__.decks()?.a.cueSend.gain.value).toBe(1);
  });
});
...
describe("MamuteEngine — demais métodos públicos", () => {
  ...
  test("setCueMonitor fecha o canal na pista e abre o envio de fone", async () => {
    ({ engine } = await createTestEngine());
    engine.setCrossfader(0);
    engine.setCueMonitor("a", true);
    ...
    expect(engine.__test__.decks()?.a.gain.gain.value).toBe(0);
    expect(engine.__test__.decks()?.a.cueSend.gain.value).toBe(1);
    engine.setCueMonitor("a", false);
    expect(engine.__test__.decks()?.a.gain.gain.value).toBeCloseTo(engine.snapshot.a.gain, 5);
    expect(engine.__test__.decks()?.a.cueSend.gain.value).toBe(0);
  });

  test("jog e quantize só tocam snapshot", async () => {
    ({ engine } = await createTestEngine());
    engine.setJogMode("a", "vinyl");
...
describe("MamuteEngine — DJ ONLINE", () => {
  test("liga MASTER no A, SYNC no B e segue o crossfader", async () => {
    ({ engine } = await createTestEngine());
    engine.setDjOnline(true);
    expect(engine.snapshot.djOnline).toBe(true);
    expect(engine.snapshot.masterDeck).toBe("a");
    expect(engine.snapshot.a.masterTempo).toBe(true);
    expect(engine.snapshot.a.sync).toBe(false);
    expect(engine.snapshot.b.sync).toBe(true);
    const masterBpm = engine.snapshot.a.bpm;
    const slaveBpm = engine.snapshot.b.bpm;
    expect(engine.snapshot.b.pitch).toBeCloseTo(((masterBpm / slaveBpm) - 1) * 100, 5);

    engine.setCrossfader(1);
    expect(engine.snapshot.masterDeck).toBe("b");
    expect(engine.snapshot.b.masterTempo).toBe(true);
    expect(engine.snapshot.a.sync).toBe(true);
    expect(engine.snapshot.b.sync).toBe(false);
  });

  test("desligar o modo não apaga o último SYNC", async () => {
    ({ engine } = await createTestEngine());
    engine.setDjOnline(true);
    engine.setDjOnline(false);
    expect(engine.snapshot.djOnline).toBe(false);
    expect(engine.snapshot.b.sync).toBe(true);
  });
});
