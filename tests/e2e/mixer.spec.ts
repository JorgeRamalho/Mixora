...
test.describe("Mixer CDJ — layout, usabilidade e acessibilidade", () => {
  ...
  test("DJ ONLINE arma MASTER/SYNC e cada deck tem LOAD MP3", async ({ page }) => {
    await page.setViewportSize(DESKTOP);
    await page.goto("/mixer");

    await expect(page.getByRole("button", { name: "DJ ONLINE" })).toBeVisible();
    await expect(page.getByRole("button", { name: "Carregar MP3 no deck A" })).toBeVisible();
    await expect(page.getByRole("button", { name: "Carregar MP3 no deck B" })).toBeVisible();

    await page.getByRole("button", { name: "DJ ONLINE" }).click();
    await expect(page.locator(".mixer-console")).toHaveAttribute("data-dj-online", "true");
    await expect(page.locator(".mixer-dj-online")).toHaveAttribute("data-harmony", "compatible");
    await expect(page.locator(".cdj-deck[data-deck='a'] .cdj-btn--master")).toHaveClass(/is-on/);
    await expect(page.locator(".cdj-deck[data-deck='b'] .cdj-btn--sync")).toHaveClass(/is-on/);
  });

  test("playlist USB: arrastar a faixa carrega no deck de destino", async ({ page }) => {
    await page.setViewportSize(DESKTOP);
    await page.goto("/mixer");
    await page.waitForSelector(".mixer-crate-track");

    const deckA = page.locator(".cdj-deck[data-deck='a']");
    const deckB = page.locator(".cdj-deck[data-deck='b']");
    await expect(page.getByRole("region", { name: "Playlist DECK A" })).toBeVisible();
    await expect(page.getByRole("region", { name: "Playlist DECK B" })).toBeVisible();
    await expect(deckA.locator(".cdj-track-meta strong")).toHaveText("Levels");

    const track = page.locator(
      ".mixer-crate-panel[data-crate-deck='a'] .mixer-crate-track[data-track-id='radio-deezer-01']",
    );
    const trackBox = await track.boundingBox();
    const deckBox = await deckB.boundingBox();
    expect(trackBox).toBeTruthy();
    expect(deckBox).toBeTruthy();
    if (!trackBox || !deckBox) return;

    await page.mouse.move(trackBox.x + trackBox.width / 2, trackBox.y + trackBox.height / 2);
    await page.mouse.down();
    await page.mouse.move(deckBox.x + deckBox.width / 2, deckBox.y + 48, { steps: 18 });
    await page.mouse.up();

    await expect(deckB.locator(".cdj-track-meta strong")).toHaveText("Titanium");
    await expect(deckA.locator(".cdj-track-meta strong")).toHaveText("Levels");
  });
});
